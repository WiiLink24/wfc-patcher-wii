...
static void ClearUserMiiInfo(mkw::NetUserHandler::Packet* packet)
{
    ...
    for (u32 i = 0; i < packet->miiGroupCount; i++) {
        ...
        u16 discard;
        if (!RFL::RFLSearchOfficialData(&packet->miiData[i].data.createID, &discard)) {
    ...
    wwfc::GPReport::ReportB64Encode("wl:mkw_user", packet, sizeof(mkw::NetUserHandler::Packet));
}
...
#  pragma GCC diagnostic ignored "-Wframe-address"
        char* output = reinterpret_cast<char*>(__builtin_frame_address(1)) + 0x18;
#  pragma GCC diagnostic pop
        return GameSpy::B64Encode(
            reinterpret_cast<char*>(&copy), output, sizeof(mkw::Nwc24PlayerInfo), 2
        );
    }
);
    ...
    [](GameSpy::GSXmlStreamWriter  stream, const char*, const char*,
       const mkw::Nwc24PlayerInfo* data) -> GameSpy::gsi_bool {
        ...
        return GameSpy::gsXmlWriteBase64BinaryElement(
            stream, "ns1", "playerinfobase64", &copy, sizeof(mkw::Nwc24PlayerInfo)
        );
    }
);
...
static void ClearMiiInfo(RFL::RFLiStoreData* miiData)
{
    ...
    std::memset(miiData->data.creatorName, 0, sizeof(miiData->data.creatorName));
    ...
    miiData->checksum = 0;
    miiData->checksum = RFL::RFLiCalculateCRC(miiData, sizeof(RFL::RFLiStoreData));
}
